...
class BlogSideBar extends Component {
	...
	handleScroll = () => {
		let fixed = this.state.positionPost < window.pageYOffset && window.pageYOffset < $('#blog').offset().top - 500;
		let footer = window.pageYOffset > $('#blog').offset().top - 500;
		console.log(window.pageYOffset, $('#blog').offset().top);
		this.setState(
			{
				fixed,
				footer,
	...
	render() {
		return (
			<section className="blog-side-bar">
				<TextField id="search-blog" label="What are you looking for?" style={{ width: '100%' }} />
				<h3>CATEGORIES</h3>
				<ul className="blog-category list-unstyled">
					{this.props.listCategoryCourse
						? this.props.listCategoryCourse.map((item, index) => {
								return (
									<li key={index}>
										<NavLink to={`/home/courses/${item.maDanhMuc}`}>{item.tenDanhMuc}</NavLink>
									</li>
								);
						  })
						: ''}
