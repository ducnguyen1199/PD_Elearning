...
class ItemCourseOfUser extends Component {
	...
	handleOnDelete = () => {
		...
		window.location.pathname === '/home/profile'
			? this.swalWarning('Bạn có muốn hủy ghi danh khóa học này không?').then(rs => {
					if (rs.value) {
			...
			: this.swalWarning('Bạn có muốn xóa khóa học này khỏi giỏ hàng không?').then(rs => {
					if (rs.value) {
			...
			<section className="item-course-of-user">
				<div className="d-flex justify-content-between align-items-center">
					...
					<div className="content-right d-flex justify-content-end align-items-center">
						...
						<div className={classnames('', { invisible: location })} onClick={this.handleOnDelete}>
